...
const Login = ({ onSubmit, onSwitchMode }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!rememberMe) {
      toast.error("You must enable 'Remember Me' to log in.");
      return;
    }
    setLoading(true);
    try {
  ...
  return (
    <div className='max-w-md w-full bg-white shadow-lg border border-purple-100 rounded-xl p-8'>
      ...
      <div className='mb-6 text-center'>
        ...
        <h2 className='text-2xl font-bold text-grey-800'>Welcom Back</h2>
        <p className='text-gray-600 text-sm mt-1'>Sign in to continue to TaskFlow</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className='space-y-4'>
         {fields.map(({name, type, placeholder, icon: Icon, isPassword}) => (
          <div key={name} className={INPUTWRAPPER}>
            ...
            {isPassword && (
              ...
              className='ml-2 text-grey-500 hover:text-purple-500 transition-colors'>
         ...
         <div className='flex items-center'>
          ...
          className='h-4 w-4 text-purple-500 focus:ring-purple-400 border-grey-300 rounded'
          required />
          <label htmlFor='rememberMe' 
              className='ml-2 block text-sm text-grey-700'>Remember Me</label>
         </div>
